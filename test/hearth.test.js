...
test('armed wake CDP discovery failures retry without exhausting attempts', async t => {
  const { hearth } = await fixture(t); hearth.config.wake.armed = true; hearth.config.wake.quiet_ms = 100; hearth.config.wake.cooldown_ms = 1000; hearth.config.wake.max_attempts = 2; const route = { conversation_key: 'openai:wake-failure', source: 'openai' }; hearth.ensureMailbox(route); let lists = 0; hearth.cdp = { listTargets: async () => { lists++; throw new Error('offline'); } }; const at = new Date().toISOString(); hearth.db.prepare('UPDATE mailboxes SET target_id=?,target_url=?,last_activity_at=?,wake_due_at=? WHERE route_key=?').run('target', 'https://chatgpt.com/c/stable', new Date(Date.now()-30000).toISOString(), new Date(Date.now()-1).toISOString(), route.conversation_key); hearth.db.prepare("INSERT INTO futures(id,route_key,kind,tool,dependencies,state,detached,at,updated_at,completed_at,result,mailbox_seq) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)").run('failure-ready', route.conversation_key, 'generic', 'machine', '[]', 'completed', 1, at, at, at, '{"state":"completed"}', 1);
  await hearth.tickRoutes(); let mailbox = hearth.db.prepare('SELECT wake_attempts,last_wake_at,wake_due_at,target_url FROM mailboxes WHERE route_key=?').get(route.conversation_key); assert.equal(mailbox.wake_attempts, 0); assert.equal(mailbox.last_wake_at, null); assert.equal(mailbox.target_url, 'https://chatgpt.com/c/stable'); assert(Date.parse(mailbox.wake_due_at) > Date.now()); await hearth.tickRoutes(); assert.equal(lists, 1);
  hearth.db.prepare('UPDATE mailboxes SET wake_due_at=? WHERE route_key=?').run(new Date(Date.now()-1).toISOString(), route.conversation_key); await hearth.tickRoutes(); mailbox = hearth.db.prepare('SELECT wake_attempts,wake_due_at,target_url FROM mailboxes WHERE route_key=?').get(route.conversation_key); assert.equal(lists, 2); assert.equal(mailbox.wake_attempts, 0); assert.equal(mailbox.target_url, 'https://chatgpt.com/c/stable'); assert(Date.parse(mailbox.wake_due_at) > Date.now());
});
...
test('armed wake validates target URL and honors quiet cooldown and attempts', async t => {
  ...
  await hearth.tickRoutes(); await waitFor(() => calls.length === 1); assert.equal(calls[0].seen.id, 'wake-target'); assert.match(calls[0].prompt, /artifact with operation list and limit 1 exactly once/); assert.match(calls[0].prompt, /without polling/); assert.match(calls[0].prompt, /Do not call machine just to wake/); assert.match(calls[0].prompt, /hearth-wake:/); await waitFor(() => !hearth.waking); await hearth.tickRoutes(); assert.equal(calls.length, 1);
  hearth.db.prepare('UPDATE mailboxes SET target_id=?,target_url=?,last_activity_at=?,last_wake_at=NULL,wake_attempts=0,wake_due_at=? WHERE route_key=?').run(target.id, target.url, new Date(Date.now() - 1000).toISOString(), new Date(Date.now() - 1).toISOString(), route.conversation_key); hearth.cdp.listTargets = async () => [{ ...target, url: 'https://chatgpt.com/c/other' }]; await hearth.tickRoutes(); assert.equal(calls.length, 1); const retained = hearth.db.prepare('SELECT target_id,target_url,wake_due_at FROM mailboxes WHERE route_key=?').get(route.conversation_key); assert.equal(retained.target_id, target.id); assert.equal(retained.target_url, target.url); assert(Date.parse(retained.wake_due_at) > Date.now());
});

test('armed wake defers a busy conversation without consuming an attempt', async t => {
  const { hearth } = await fixture(t); hearth.config.wake.armed = true; hearth.config.wake.quiet_ms = 100; hearth.config.wake.cooldown_ms = 1000; hearth.config.wake.max_attempts = 2; const route = { conversation_key: 'openai:wake-busy', source: 'openai' }; hearth.ensureMailbox(route); const target = { id: 'busy-target', url: 'https://chatgpt.com/c/busy', webSocketDebuggerUrl: 'ws://127.0.0.1:9223/busy' }; const at = new Date().toISOString();
  hearth.cdp = { listTargets: async () => [target], contains: async () => false, send: async () => ({ state: 'busy', reason: 'still generating' }) }; hearth.db.prepare('UPDATE mailboxes SET target_id=?,target_url=?,bound_at=?,last_activity_at=?,wake_due_at=? WHERE route_key=?').run(target.id, target.url, at, new Date(Date.now()-1000).toISOString(), new Date(Date.now()-1).toISOString(), route.conversation_key); hearth.db.prepare("INSERT INTO futures(id,route_key,kind,tool,dependencies,state,detached,at,updated_at,completed_at,result,mailbox_seq) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)").run('busy-ready', route.conversation_key, 'generic', 'machine', '[]', 'completed', 1, at, at, at, '{\"state\":\"completed\"}', 1);
  await hearth.tickRoutes(); await waitFor(() => !hearth.waking); const mailbox = hearth.db.prepare('SELECT wake_attempts,wake_due_at,offered_seq FROM mailboxes WHERE route_key=?').get(route.conversation_key); assert.equal(mailbox.wake_attempts, 0); assert.equal(mailbox.offered_seq, 0); assert(Date.parse(mailbox.wake_due_at) > Date.now()); const event = hearth.db.prepare("SELECT data FROM events WHERE kind='route.wake' ORDER BY id DESC LIMIT 1").get(); assert.equal(JSON.parse(event.data).state, 'deferred');
});

test('armed wake rebinds a restarted Chrome target by stable conversation URL', async t => {
  const { hearth } = await fixture(t); hearth.config.wake.armed = true; hearth.config.wake.quiet_ms = 100; hearth.config.wake.cooldown_ms = 1000; const route = { conversation_key: 'openai:rebind', source: 'openai' }; hearth.ensureMailbox(route); const url = 'https://chatgpt.com/c/rebind'; const replacement = { id: 'new-target', url, webSocketDebuggerUrl: 'ws://127.0.0.1:9223/new-target' }; const calls = []; const at = new Date().toISOString();
  hearth.cdp = { listTargets: async () => [replacement], contains: async () => false, send: async (seen) => { calls.push(seen); hearth.db.prepare('UPDATE mailboxes SET offered_seq=1 WHERE route_key=?').run(route.conversation_key); return { state: 'completed' }; } };
  hearth.db.prepare('UPDATE mailboxes SET target_id=?,target_url=?,bound_at=?,last_activity_at=?,wake_due_at=? WHERE route_key=?').run('stale-target', url, at, new Date(Date.now()-1000).toISOString(), new Date(Date.now()-1).toISOString(), route.conversation_key); hearth.db.prepare("INSERT INTO futures(id,route_key,kind,tool,dependencies,state,detached,at,updated_at,completed_at,result,mailbox_seq) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)").run('rebind-ready', route.conversation_key, 'generic', 'machine', '[]', 'completed', 1, at, at, at, '{\"state\":\"completed\"}', 1);
  await hearth.tickRoutes(); await waitFor(() => calls.length === 1); await waitFor(() => !hearth.waking); const mailbox = hearth.db.prepare('SELECT target_id,target_url FROM mailboxes WHERE route_key=?').get(route.conversation_key); assert.equal(mailbox.target_id, 'new-target'); assert.equal(mailbox.target_url, url); assert.equal(calls[0].id, 'new-target'); assert(hearth.db.prepare("SELECT id FROM events WHERE kind='route.rebound' ORDER BY id DESC LIMIT 1").get());
});
...
test('CDP wake writes the exact bounded prompt and submits it', async () => {
  ...
  class FakeWebSocket {
    ...
    send(body) {
      ...
      let value = true;
      if (request.method === 'Runtime.evaluate' && request.params?.expression?.includes('composerAvailable')) value = { composerAvailable: true, busy: false };
      else if (request.method === 'Runtime.evaluate' && request.params?.expression?.includes('inComposer')) value = { inComposer: false, inConversation: false };
  ...
  try {
    ...
    assert.deepEqual(requests.map(request => request.method), ['Runtime.evaluate','Runtime.evaluate','Input.insertText','Runtime.evaluate','Input.dispatchKeyEvent','Input.dispatchKeyEvent','Runtime.evaluate']);
    assert.match(requests[0].params.expression, /stop-button/); assert.equal(requests[0].params.expression.includes('innerText'), false);
    assert.equal(requests[2].params.text, prompt); assert.equal(requests[4].params.type, 'rawKeyDown');
  } finally { globalThis.WebSocket = OriginalWebSocket; }
});

test('CDP wake defers without touching the composer while ChatGPT is generating', async () => {
  const OriginalWebSocket = globalThis.WebSocket; const requests = [];
  class FakeWebSocket {
    listeners = new Map();
    addEventListener(name, fn) { if (!this.listeners.has(name)) this.listeners.set(name, []); this.listeners.get(name).push(fn); if (name === 'open') queueMicrotask(fn); }
    send(body) { const request = JSON.parse(body); requests.push(request); const result = { result: { value: { composerAvailable: true, busy: true } } }; queueMicrotask(() => (this.listeners.get('message') || []).forEach(fn => fn({ data: JSON.stringify({ id: request.id, result }) }))); }
    close() {}
  }
  globalThis.WebSocket = FakeWebSocket;
  try {
    const result = await sendChatPrompt({ url: 'https://chatgpt.com/c/busy-test', webSocketDebuggerUrl: 'ws://127.0.0.1:9223/devtools/page/busy' }, 'wake', 'http://127.0.0.1:9223', 1000);
    assert.equal(result.state, 'busy'); assert.match(result.reason, /generating/); assert.deepEqual(requests.map(request => request.method), ['Runtime.evaluate']);
  } finally { globalThis.WebSocket = OriginalWebSocket; }
});
